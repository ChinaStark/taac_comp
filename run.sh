...
ensure_project_pip_dependencies() {
	...
	ensure_python
	if [[ "${BUNDLE_MODE}" == "1" && -z "${TAAC_BUNDLE_PIP_EXTRAS:-}" ]]; then
		export TAAC_BUNDLE_PIP_EXTRAS="${SUPPORTED_CUDA_PROFILE}"
	fi
...
extra_index_urls = shlex.split(os.environ.get("TAAC_PIP_EXTRA_INDEX_URLS", ""))
if not extra_index_urls and "cuda126" in extras:
    extra_index_urls.append("https://download.pytorch.org/whl/cu126")
