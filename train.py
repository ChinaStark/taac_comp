...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--tf_events_dir', type=str, default=None,
                        help='TensorBoard event directory (env: TRAIN_TF_EVENTS_PATH)')
    ...
    if not args.ckpt_dir:
        args.ckpt_dir = os.path.join(default_outputs_dir, 'checkpoints')
    if not args.log_dir:
        args.log_dir = os.path.join(default_outputs_dir, 'logs')
    if not args.tf_events_dir:
        args.tf_events_dir = os.path.join(default_outputs_dir, 'tf_events')
...
def main() -> None:
    ...
    if not args.data_dir:
        raise ValueError(
            "Training data path is required. Set --data_dir or env TRAIN_DATA_PATH."
        )
