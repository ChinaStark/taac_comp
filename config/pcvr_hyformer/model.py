...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        emb_dim: int = 64,
        num_queries: int = 1,
        num_hyformer_blocks: Optional[int] = None,
        ...
        if num_hyformer_blocks is None:
            num_hyformer_blocks = 2 if num_blocks is None else int(num_blocks)
        elif num_blocks is not None and int(num_blocks) != int(num_hyformer_blocks):
            logging.warning(
                "Received both num_hyformer_blocks=%s and num_blocks=%s; using num_hyformer_blocks",
                num_hyformer_blocks,
                num_blocks,
            )
    ...
    def get_embeddings(self, inputs: ModelInput) -> torch.Tensor:
        """Returns the penultimate representation used for prediction."""
        _logits, embeddings = self.predict(inputs)
        return embeddings
